...
object Server extends ServerModuleInterface:
  ...
  val identityParserSink: Sink[ByteString, Future[Identity]] =
    reframedFlow
      .map(Identity.parse)
      .toMat(Sink.head[Identity])(Keep.right)
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] =
    /*
    Test description: reintroduceOrdering: 1000 events in random order (3pts)(followers.FollowersSuite)
    ...
    Flow[Event]
      .statefulMapConcat{() =>
        var expectedSeq = 1
        var buffer: List[Event] = Nil
        elem => {
          if (elem.sequenceNr > expectedSeq) {
            buffer = elem :: buffer
            Nil
          } else {
            expectedSeq += 1
            buffer.foreach{e =>
              if (e.sequenceNr <= expectedSeq) {
                expectedSeq = e.sequenceNr + 1
              }
            }
            val toAppend = buffer.filter(e => e.sequenceNr <= expectedSeq)
            buffer = buffer.filter(e => e.sequenceNr > expectedSeq)
            elem :: toAppend
          }
        }
      }
  ...
  val followersFlow: Flow[Event, (Event, Followers), NotUsed] =
    reintroduceOrdering
      .statefulMapConcat{() =>
        var followerMap = Map.empty[Int, Set[Int]]
        elem => {
          elem match {
            case Event.Follow(_, fromUserId, toUserId) =>
              if (followerMap.contains(fromUserId))
                followerMap = followerMap + (fromUserId -> (followerMap(fromUserId) + toUserId))
              else
                followerMap = followerMap + (fromUserId -> Set(toUserId))
            case Event.Unfollow(_, fromUserId, toUserId) =>
              if (followerMap.contains(fromUserId)) {
                val updatedSet = followerMap(fromUserId) - toUserId
                followerMap = followerMap + (fromUserId -> updatedSet)
              }
            case _ =>
          }
          (elem, followerMap) :: Nil
        }
      }
  ...
  def isNotified(userId: Int)(eventAndFollowers: (Event, Followers)): Boolean =
    ...
    event match {
      ...
      case Event.StatusUpdate(_, fromUserId) => followers.getOrElse(userId, Set.empty).contains(fromUserId)
      case _ => false
    }
...
class Server(using ExecutionContext, Materializer)
  ...
  val (inboundSink, broadcastOut) =
    ...
    val incomingDataFlow: Flow[ByteString, (Event, Followers), NotUsed] =
      eventParserFlow via followersFlow
    ...
    val incoming: Sink[ByteString, NotUsed] =
      identityParserSink
        .mapMaterializedValue{ m =>
          clientIdPromise.completeWith(m)
          NotUsed
        }
