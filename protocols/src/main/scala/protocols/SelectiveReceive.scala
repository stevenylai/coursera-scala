...
object SelectiveReceive:
  ...
  def apply[T: ClassTag](bufferCapacity: Int, initialBehavior: Behavior[T]): Behavior[T] =
    Behaviors
      .withStash(bufferCapacity)(buffer =>
    ...
    Behaviors.receive { case (ctx, message) =>
      ...
      try {
        ...
        if (Behavior.isUnhandled(next)) {
          ...
          //println(s"stashed ${message} to ${buffer} ${started} ${next} ${Behavior.canonicalize(started, next, ctx)}")
          intercept(bufferSize, buffer, Behavior.canonicalize(next, started, ctx))
        } else {
          val canonicalBehavior = Behavior.canonicalize(next, started, ctx)
          //println(s"unstashed ${buffer} ${started} ${next} ${canonicalBehavior}")
          buffer.unstashAll(SelectiveReceive(bufferSize, canonicalBehavior))
        }
      } catch {
        case _: IllegalArgumentException =>
          println("xxxxxxxxxxxxxx")
          SelectiveReceive(bufferSize, Behavior.validateAsInitial(started))
      }
    }
