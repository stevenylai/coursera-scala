...
class HuffmanSuite {
  ...
  @Test def `decode and encode a very short text should be identity (10pts)`: Unit =
    new TestTrees {
      ...
      val text = "Solution: names of variables should start with a lower-case letter."
      //println(times(text.toList))
      val code = createCodeTree(text.toList)
      //println(code)
      assertEquals(text.toList, decode(code, encode(code)(text.toList)))
      //println(code)
      //println(convert(code))
      assertEquals(encode(code)(text.toList), quickEncode(code)(text.toList))
    }
