...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    b.legalNeighbors.foldLeft(LazyList[(Block, List[Move])]()) {
      case (res, (curBlock, curHistory)) => (curBlock, curHistory :: history) #:: res
    }
  }
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors filter {
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    val more = for {
      start <- initial
      next <- newNeighborsOnly(neighborsWithHistory(start._1, start._2), explored)
    } yield next
    val newExplored = more.foldLeft(Set[Block]()){
      case (curExplored, (block, _)) => curExplored + block
    }
    if (newExplored.isEmpty) initial else initial ++ more ++ from(more, explored ++ newExplored)
  }
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = {
    pathsFromStart.foldLeft(LazyList[(Block, List[Move])]()) {
      case (res, (block, history)) => if (!done(block)) res else (block, history) #:: res
    }
  }
   ...
   * position.
   */
  lazy val solution: List[Move] = if (pathsToGoal.isEmpty) Nil else pathsToGoal.take(1).head._2.reverse
}
