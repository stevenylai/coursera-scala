...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val grouped = columnNames.groupBy{ s =>
      if (s.startsWith("t01") || s.startsWith("t03") || s.startsWith("t11") ||
      s.startsWith("t1801") || s.startsWith("t1803")) "primary"
      else if (s.startsWith("t05") || s.startsWith("t1805")) "working"
      else if (s.startsWith("t02") || s.startsWith("t04") || s.startsWith("t06") ||
      s.startsWith("t07") || s.startsWith("t08") || s.startsWith("t09") ||
      s.startsWith("t10") || s.startsWith("t12") || s.startsWith("t13") ||
      s.startsWith("t14") || s.startsWith("t15") || s.startsWith("t16") ||
      s.startsWith("t18")) "leisure"
      else "unknown"
    }
    (grouped("primary").map{new Column(_)},
      grouped("working").map{new Column(_)},
      grouped("leisure").map{new Column(_)})
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when($"telfs" >= 0 && $"telfs" < 3, "working")
